...
mod test {
    ...
    fn workspace_will_rename_files_capabilities_simple() -> ServerCapabilities {
        ServerCapabilities {
            workspace: Some(WorkspaceServerCapabilities {
                workspace_folders: None,
                file_operations: Some(WorkspaceFileOperationsServerCapabilities {
    ...
    fn test_server_rename_simple_expect_none_got_none() {
        ...
        send_response_num(NON_RESPONSE_NUM, &test_case_root).expect("Failed to send response num");
        send_capabiltiies(
            &workspace_will_rename_files_capabilities_simple(),
            &test_case_root,
        )
        .expect("Failed to send capabilities");
    ...
    fn test_server_rename_simple_expect_none_got_some(
        ...
        // NOTE: The URI passed here matches the cleaned URI in the response
        let resp = test_server::responses::get_workspace_will_rename_files_response(
            response_num,
            &Uri::from_str("main.dummy").unwrap(),
        )
        .unwrap();
    ...
    fn test_server_rename_simple_expect_some_got_some(
        ...
        let uri = Uri::from_str(&test_server::get_dummy_source_path()).unwrap();
        let resp =
            test_server::responses::get_workspace_will_rename_files_response(response_num, &uri)
                .unwrap();
        let source_file = TestFile::new(test_server::get_dummy_source_path(), "");
        let test_case = TestCase::new(get_dummy_server_path(), source_file);
