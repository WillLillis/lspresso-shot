...
mod test {
    ...
    use lsp_types::{LinkedEditingRangeServerCapabilities, Position, ServerCapabilities, Uri};
    use rstest::rstest;

    fn linked_editing_range_capabilities_simple() -> ServerCapabilities {
        ServerCapabilities {
    ...
    fn test_server_simple_expect_none_got_some(#[values(0, 1, 2, 3)] response_num: u32) {
        ...
        let resp =
            test_server::responses::get_linked_editing_range_response(response_num, &uri).unwrap();
        let source_file = TestFile::new(test_server::get_dummy_source_path(), "");
        let test_case = TestCase::new(get_dummy_server_path(), source_file);
    ...
    // NOTE: rust-analyzer doesn't support `textDocument/linkedEditingRange` requests
}
