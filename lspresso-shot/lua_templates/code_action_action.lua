...
RANGE
    ...
    local code_action_result = vim.lsp.buf_request_sync(0, 'textDocument/codeAction', {
    ...
    if not code_action_result then
        ...
        report_log('No valid code action result returned: ' .. vim.inspect(code_action_result) .. '\n')
    elseif code_action_result and #code_action_result >= 1 and code_action_result[1].result then
        ...
        results_file:write(vim.json.encode(code_action_result[1].result, { escape_slash = true }))
        results_file:close()
